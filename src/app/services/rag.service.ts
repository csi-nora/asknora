...
export class RagService {
  ...
  async indexDocuments(docs: KbDocument[]): Promise<void> {
    // Full rebuild so removed docs and replaced corpus do not leave stale chunks
    this._chunks.clear();
    this._vectors.clear();
    this.bm25.build([]);
  ...
  clearAll(): void {
    ...
    this._vectors.clear();
    this.bm25.build([]);
    this.storage.del(LS_CHUNKS);
    this.storage.del(LS_VECTORS);
    this._syncStats();
  }
  ...
  // ── Storage (via StorageService: localStorage first, IndexedDB if quota exceeded) ──
  private _saveToStorage(): void {
    try {
      this.storage.setStringWithFallback(LS_CHUNKS, JSON.stringify([...this._chunks.values()]));
      this.storage.setStringWithFallback(LS_VECTORS, JSON.stringify([...this._vectors.entries()]));
    } catch (e) {
  ...
  private _loadFromStorage(): void {
    try {
      const rc = this.storage.getStringWithFallback(LS_CHUNKS);
      if (rc) {
        const chunks: TextChunk[] = JSON.parse(rc);
        this._chunks = new Map(chunks.map(c => [c.id, c]));
        this.bm25.build(chunks);
      }
      const rv = this.storage.getStringWithFallback(LS_VECTORS);
      if (rv) {
      ...
      console.warn('RAG storage load failed', e);
    }
  }
  ...
  storageBytes(): number {
    return this.storage.bytes(LS_CHUNKS) + this.storage.bytes(LS_VECTORS);
  }
}

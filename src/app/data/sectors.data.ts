import { KbDocument, Sector } from '../models';
...
/** Keyword hints used when a doc lacks an explicit sector / businessSector field. */
const SECTOR_HINTS: Record<string, string[]> = {
  healthcare:    ['healthcare', 'hospital', 'clinic', 'iomt', 'ehr', 'hipaa', 'patient', 'medical', 'pharma'],
  financial:     ['financial', 'bank', 'fintech', 'mas trm', 'insurance', 'swift', 'trading', 'fintech'],
  government:    ['government', 'govtech', 'cii', 'public sector', 'ministry', 'agency', 'wog'],
  retail:        ['retail', 'e-commerce', 'ecommerce', 'pci-dss', 'pci dss', 'loyalty', 'pos '],
  manufacturing: ['manufacturing', 'factory', 'ot/ics', 'ics ', 'plc', 'industry 4.0', 'scada'],
  logistics:     ['logistics', 'maritime', 'port ', 'vessel', 'shipping', 'supply chain', 'ais/gps'],
  telco:         ['telco', 'telecom', '5g core', 'ss7', 'network slicing', 'imda', 'broadcast', 'ott'],
  sme:           ['sme', 'enterprise', 'cyber essentials', 'smb', 'mid-market'],
};

/**
 * Resolve which business-sector key a KB document belongs to.
 * Prefers explicit `sector` / `businessSector`; otherwise infers from name+content.
 */
export function resolveDocSector(doc: Pick<KbDocument, 'sector' | 'businessSector' | 'name' | 'content'>): string {
  const explicit = (doc.sector || doc.businessSector || '').trim().toLowerCase();
  if (explicit && SECTORS[explicit]) return explicit;

  const hay = `${doc.name || ''} ${(doc.content || '').slice(0, 800)}`.toLowerCase();
  let best = 'sme';
  let bestHits = 0;
  for (const [key, hints] of Object.entries(SECTOR_HINTS)) {
    let hits = 0;
...
/**
 * Live RAG chunk counts per sector key, derived from the active KB doc list.
 * Optionally filter by sensitivity ACL (role-visible docs only).
 */
export function chunkCountsBySector(
  docs: KbDocument[],
  canAccess?: (sensitivity: KbDocument['sensitivity']) => boolean,
): Record<string, number> {
  const out: Record<string, number> = {};
  for (const k of SECTOR_KEYS) out[k] = 0;
  for (const d of docs) {
    if (canAccess && !canAccess(d.sensitivity)) continue;
    const key = resolveDocSector(d);
    out[key] = (out[key] || 0) + (d.chunkCount || 0);
  }
  return out;
}
